...
void *worker_thread(void *arg) {
    ...
    while (true) {
        ...
        size_t to_receive = sizeof(query_data);
        unsigned char *buf = reinterpret_cast<unsigned char*>(&inbound_data);

    while (total_received < to_receive) {
        ssize_t chunk = read(client_sock, buf + total_received, to_receive - total_received);
        if (chunk < 0) {
            if (errno == EINTR) {
                continue;
            }
                ...
                switch (inbound_data.real_type) {
                    ...
                    case GET_FULL_USER_BY_UNAME: {
                        ...
                        while (sqlite3_step(stmt) == SQLITE_ROW) {
                            ...
                            size_t to_send = sizeof(full_user_data);
                            while (total_sent < to_send) {
                                ssize_t sent = send(client_sock, ((char*)&user_data) + total_sent, to_send - total_sent, 0);
                                if (sent <= 0) {
                                    perror("send error");
                                    break;
                                }
                            ...
                            shutdown(client_sock, SHUT_WR); // Gracefully signal end-of-data.
